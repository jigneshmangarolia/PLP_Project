# Jignesh Mangarolia (CE 4337.502) and Tisha Arora (CS 4337.501) 
...
# for handling the plus sign over all of the test example files given
def handlePlus(input):
    if len(input) > 1:
        if input[1].isdigit():
            return lexInt(input[1:], 1)
        elif input[2].isspace() or input[1] == "\n":
            return [[ERROR, "Expected variable/digit after '+' on line " + str(line)], input]
        elif input[1] == "_" or input[1].isalpha():
...
# for handling elements which were in quotes "", and what to do while the elements are in quotes
def handleString(input):
    i = 1
    while i < len(input):
        i = i + 1
        if input[i] == "\"": # for \n and to break after it to end the string
            i = i + 1
            break
    return [[STRING_TOKEN, str("".join(input[0:i]))], input[i:]]
